...
public interface FilterDsl {
	 ...
	 * expressions.
	 * <p>
	 * {@code Emitter} defines the primitive operations that all backend compilers
	 * must implement. The filter DSL ({@link PacketDsl} and {@link HeaderDsl})
	 * emits calls to this interface, and each backend translates them into its
	 * native syntax:
	 * <ul>
	 * <li>{@code BpfFilterBuilder} - libpcap BPF expressions</li>
	 * <li>{@code RteFlowBuilder} - DPDK rte_flow
	 * 
	 * patterns (C structs)</li>
	...
	public interface Emitter {
		 ...
		 * @return this builder for chaining
		 */
		Emitter portRange(int start, int end);
		 ...
		 * </p>
		 * <p>
		 * The special keyword {@link PacketFilter#KEYWORD_ALL} ({@code "all"}) signals
		 * an unconditional match-all filter. Backend builders handle this case
		 * specifically: some emit a native accept-all directive, others treat it as a
		 * no-op.
		 * </p>
		 *
		 * @param protocol protocol name, or {@link PacketFilter#KEYWORD_ALL} for
		 *                 catch-all
		 * @return this builder for chaining
		 */
		Emitter protocol(String protocol);

		/**
		 * Adds a protocol presence check at a specific stack depth.
		 * <p>
		 * Useful for tunneled or stacked protocols (e.g. inner vs. outer VLAN, MPLS
		 * label depth).
		 * </p>
		 *
		 ...
		 * @return this builder for chaining
		 */
		Emitter protocol(String protocol, int depth);
		 ...
		 * @return this builder for chaining
		 */
		Emitter field(String name, int offset, int bits, Op op, long value);
		 ...
		 * @return this builder for chaining
		 */
		Emitter field(String name, int offset, int bits, Op op, byte[] value);
		 ...
		 * @return this builder for chaining
		 */
		Emitter onExpressionAction(Consumer<String> debugAction, Consumer<Registration> registration);

		/**
		 * Builds and returns the final filter expression string.
		 * <p>
		 * The format of the returned string is backend-specific (e.g. BPF syntax, C
		 * struct declarations, NTPL commands). Any registered expression action
