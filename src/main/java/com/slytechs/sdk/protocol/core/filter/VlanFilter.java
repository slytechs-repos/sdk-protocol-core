 ...
 * VlanDsl filter1 = VlanFilter.vid(100);
 *
 * // Combined conditions (fluent builder)
 * VlanDsl filter2 = VlanFilter.of()
 * 		.pcp(5)
 * 		.dei(0)
 * 		.vid(200)
 * 		.type(0x8100);
 ...
 * VlanDsl filter3 = VlanFilter.pcp(3)
 * 		.vid(4094)
...
public interface VlanFilter {
	 ...
	 *
	 * @return a new {@link VlanDsl} instance with no filters applied
	 */
	static VlanDsl of() {
		return b -> b;
	}
	 ...
	 * @return a {@link VlanDsl} configured with the PCP condition
	 * @throws FilterException if priority is not in the range 0–7
	 */
	static VlanDsl pcp(int priority) throws FilterException {
	 ...
	 * @return a {@link VlanDsl} configured with the DEI condition
	 * @throws FilterException if dropEligible is not 0 or 1
	 */
	static VlanDsl dei(int dropEligible) throws FilterException {
	 ...
	 * @return a {@link VlanDsl} configured with the VID condition
	 * @throws FilterException if vid is not in the range 0–4095
	 */
	static VlanDsl vid(int vid) throws FilterException {
		if (vid < 0 || vid > 4095) {
	 ...
	 * @return a {@link VlanDsl} configured with the type/TPID condition
	 ...
	 * </p>
	 */
	interface VlanDsl extends HeaderDsl {
		 ...
		 * @throws FilterException if priority is not in the range 0–7
		 */
		default VlanDsl pcp(int priority) throws FilterException {
			if (priority < 0 || priority > 7) {
				throw new FilterException("PCP (priority) must be 0-7, got: " + priority);
			}
		 ...
		 * @throws FilterException if dropEligible is not 0 or 1
		 */
		default VlanDsl dei(int dropEligible) throws FilterException {
			if (dropEligible < 0 || dropEligible > 1) {
				throw new FilterException("DEI (drop eligible) must be 0-1, got: " + dropEligible);
			}
		 ...
		 * @throws FilterException if vid is not in the range 0–4095
		 */
		default VlanDsl vid(int vid) throws FilterException {
			if (vid < 0 || vid > 4095) {
				throw new FilterException("VLAN ID must be 0-4095, got: " + vid);
			}
