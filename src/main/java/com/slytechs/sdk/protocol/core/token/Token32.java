 ...
 */
public record Token32(int token32) implements Token {

	public Token32(int status, int lod, int domain, int type) {
		this(Tokens.token32(4, status, lod, domain, type));
	}

	public Token32(int domain, int type) {
		this(Tokens.token32(4, Token.NORMAL_STATUS, Token.NORMAL_LOD, domain, type));
	}
	...
	public int write(MemorySegment seg, long offset) {
		return Tokens.u32(seg, offset, token32);
	}
	...
	@Override
	public long token64() {
		return Integer.toUnsignedLong(token32);
	}
